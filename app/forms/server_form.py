...
class ServerForm(FlaskForm):
    displayname = StringField('Server Name', validators=[DataRequired(), Length(max=40, message="Length of server name must be less than 40 characters")])
    icon = URLField("icon", validators=[Length(max=256, message="Length of URL must be less than 256 characters")])
    desc = StringField('description', validators=[Length(max=512, message="Length of description must be less than 256 characters")])
